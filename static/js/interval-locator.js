...
(function() {
    ...
    const WRONG_FLASH_MS = 700;  // how long a wrong cell flashes red
    ...
    // ---- Matrix renderer (cells are the input) ----

    function renderMatrix(container, N, centerSemitone, onCellClick) {
        ...
        for (let row = 0; row < N; row++) {
            ...
            for (let col = 0; col < N; col++) {
                ...
                if (isCenter) {
                    ...
                    g.addEventListener('click', () => onCellClick(row, col));
                    cellEls[row][col] = {
                        rect,
                        text,
    ...
    // ---- Chromatic circle prompt (display only) ----

    function renderChromaticCircle(container, centerSemitone, targetSemitone) {
        const svgNS = 'http://www.w3.org/2000/svg';
        const size = 400;
        const svg = document.createElementNS(svgNS, 'svg');
        ...
        for (let i = 0; i < 12; i++) {
            ...
            const isTarget = (i === targetSemitone);
    ...
    function renderGameView() {
        ...
        // Matrix half (left / top) — the input
        ...
        // Circle half (right / bottom) — the prompt
